                        ...
                        .then(CommandManager.argument("message", StringArgumentType.greedyString())
                                .executes(context -> {
                                    ...
                                    Mappings.sendFeedback(context,
                                            Mappings.literalText(String.format("§6[me §r-> §6%s]: §r%s",
                                                    target.getGameProfile().getName(), message)),
                                            false);
                                    target.sendMessage(
                                            Mappings.literalText(
                                                    String.format("§6[%s §r-> §6me]: §r%s", playerName, message)),
                                            false);
                                    ...
                                    target.playSound(SoundEvents.ENTITY_EXPERIENCE_ORB_PICKUP, SoundCategory.MASTER, 1,
                                            1);

                                    /*
                                     * if(afk.afkList.containsKey(target.getUuid()) &&
                                     * context.getSource().isExecutedByPlayer()) {
                                     * context.getSource().getPlayer().sendMessageToClient(Mappings.literalText("").
                                     * append(target.getDisplayName()).
                                     * append(" are AFK and may not be available at the moment.").formatted(
